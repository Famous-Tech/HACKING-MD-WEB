  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://hacking-md-pairing.onrender.com/api/auth?mode=pair&number=${phoneNumber}`
      );
      ...
      <div className="max-w-md mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-300">
            Sélectionnez votre pays et entrez votre numéro pour recevoir un code
          </p>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg shadow-xl p-6">
          ...
          {pairCode && (
            <div className="mt-6 p-4 bg-gray-700 rounded-lg">
              <p className="text-sm text-gray-300 mb-2">Votre code d'appairage :</p>
              <p className="text-2xl font-mono text-white text-center tracking-wider">
                {pairCode}
              </p>
              <p className="mt-2 text-xs text-gray-400 text-center">
                Ce code est à usage unique et expirera après utilisation
              </p>
            </div>
          )}
        </div>
